...
{
    /// <inheritdoc />
    public partial class uspGetAllHubsupdated2 : Migration
    {
        /// <inheritdoc />
...
ALTER PROCEDURE [dbo].[usp_GetAllHubs]
	...
	SET NOCOUNT ON;
		SELECT(
			...
			WHERE h.IsDeleted=0
				 ...
				 AND (@Phone='""""' OR Contains(h.Phone,@Phone))
				 AND (@HubTypeId IS NULL OR h.HubTypeId=@HubTypeId)
				 AND (@ZipCode IS NULL OR h.Id IN(select hz.HubId from HubZipCodeMaps hz
												   LEFT JOIN ZipCodes z on hz.ZipCode=z.Id
												   where hz.ZipCode=@ZipCode OR
												   z.Name=@ZipCode))
			...
			s.Name as StateName,
			ci.Name as CityName,
			ht.Name as HubTypeName,
			ml.BinaryObjectId,
			(select COUNT(hs.StoreId) from HubStores hs where hs.HubId=h.Id) as NumberOfStores,
			(select COUNT(hc.ContactId) from HubContacts hc where hc.HubId=h.Id) as NumberOfContacts,
			(select COUNT(hb.BusinessId) from HubBusinesses hb where hb.HubId=h.Id) as NumberOfBusinesses
			from [dbo].[Hubs] h
			LEFT JOIN [dbo].[HubTypes] ht on h.HubTypeId=ht.Id
			LEFT JOIN [dbo].[Countries] con on h.CountryId=con.Id
			LEFT JOIN [dbo].[States] s on h.StateId=s.Id
